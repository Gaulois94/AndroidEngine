...
void Menu::clearItems(bool del)
{
	for(auto item : m_itemsMenu)
	{
		if(del)
			delete item;
		else
		{
			item->setParent(NULL);
			item->setApplyTransformation(NULL);
		}
	}
...
void Menu::updateConfiguration()
{
	...
	//Update the position of everyone
	for(ItemMenu* item : m_itemsMenu)
	{
		item->setPosition(glm::vec3(0.0, -size.y, 0.0));
		g = item->getRect().getSize();
		size.x = fmax(size.x, g.x);
		size.y += g.y;
	}

	//Then update the position origin
	for(ItemMenu* item : m_itemsMenu)
	{
		switch(m_itemOrigin)
		{
			case X_LEFT:
				item->setDefaultPositionOrigin(BOTTOM_LEFT);
				item->setPosition(glm::vec3(0, -size.y, 0));
				break;
			case X_CENTER:
				item->setDefaultPositionOrigin(BOTTOM_CENTER);
				item->setPosition(glm::vec3(size.x/2.0f, -size.y, 0));
				break;
			case X_RIGHT:
				item->setDefaultPositionOrigin(BOTTOM_RIGHT);
				item->setPosition(glm::vec3(0, -size.y, 0));
				break;
		}
	}
